...
void doTranspose(int n_rows, int n_cols,
                 const char *in_file, int in_file_stride,
                 char *out_file, int out_file_stride,
                 Options *opt);
...
int main(int argc, char **argv) {
  ...
  u64 in_file_len, out_file_len = 0;
  char *out_file;
  const char *in_file;
  Options opt;
  ...
  opt.move_right = 0;
  opt.read_block_height = 64;
  opt.read_block_width = 64;
  ...
  out_file_len = n_cols * (n_rows + newline_len);
  ...
  doTranspose(n_rows, n_cols, in_file, n_cols + newline_len,
              out_file, n_rows + newline_len, &opt);
...
void doTranspose(int n_rows, int n_cols,
  ...
  int block_left, block_top, x, y, block_right, block_bottom;

  for (block_left = 0; block_left < n_cols;
       block_left += opt->read_block_width) {

    block_right = MIN(block_left + opt->read_block_width, n_cols);

    for (block_top = 0; block_top < n_rows;
         block_top += opt->read_block_height) {

      block_bottom = MIN(block_top + opt->read_block_height, n_rows);

      for (x = block_left; x < block_right; x++) {
        for (y = block_top; y < block_bottom; y++) {

          out_file[x * out_file_stride + y] = in_file[y * in_file_stride + x];

          /* printf("in(%d,%d) -> out(%d,%d)\n", y, x, x, y); */

        }

        if (block_bottom == n_rows) {
          writeNewline(out_file + x * out_file_stride + n_rows,
                       opt->newline_type);
        }
